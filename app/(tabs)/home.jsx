...
const Home = () => {
  ...
  return (
    <ScrollView className="bg-white h-full bg-[#000] pt-6">
      ...
      <Text className="mt-1 mx-3 mb-10 font-psemibold text-[16px] text-[#ffffff90] text-center absolute top-0">физкультпривет, {user?.name}</Text>
...
<View key={0} className="relative w-[90vw] mr-[3vw] h-[135px] bg-[#161616] px-4 py-2 rounded-2xl overflow-hidden mb-4">
...
<Text className="text-[#838383] text-center font-pregular text-[15px] mt-[8px]">создать тренировку</Text>
            ...
            <ScrollView horizontal={true} 
              ...
              decelerationRate="fast" // Быстрая инерция прокрутки
              showsHorizontalScrollIndicator={false} // Отключаем горизонтальную полосу прокрутки
              className="relative w-[100vw] h-[165px] rounded-2xl pl-4">
            {trackers.map(track => {
              const contentList = [
                {
            ...
            <Text className="text-xl font-pbold relative text-[#fff] mt-[16px] text-center mb-4">друзья и блогеры</Text>
            <ScrollView horizontal={true} 
        snapToInterval={(width * 0.9225)} // Устанавливаем величину для "щелчка" на следующий элемент
        decelerationRate="fast" // Быстрая инерция прокрутки
        showsHorizontalScrollIndicator={false} // Отключаем горизонтальную полосу прокрутки
            ...
            {users.map(user => {
              ...
              return(
                ...
                className="h-full absolute left-0 top-0 w-[50vw] z-10" colors={['#fff0', '#161616']}></LinearGradient>
